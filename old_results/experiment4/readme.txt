Make predictions based on current state (indicators) and past state
...
class Encoder(nn.Module):
    def __init__(self, inchannel, state_length, immediate_state_dim, hidden_size, outchannel, activation=nn.ReLU):
        ...
        if inchannel == outchannel:
            ...
            self.shortcut1 = nn.Conv1d(inchannel, outchannel, kernel_size=1)

        self.layers2 = nn.Sequential(
            nn.Conv1d(immediate_state_dim[0], hidden_size, kernel_size=3, padding=1),
            nn.BatchNorm1d(hidden_size),
            activation(),
            nn.Conv1d(hidden_size, outchannel, kernel_size=3, padding=1),
            nn.BatchNorm1d(outchannel),
        ...
        if inchannel == outchannel:
            ...
            self.shortcut2 = nn.Conv1d(immediate_state_dim[0], outchannel, kernel_size=1)

        self.output = nn.Linear(outchannel * state_length + outchannel * immediate_state_dim[1], outchannel)
    
    def forward(self, X, X_immediate):
        out =  self.layers1(X)
        shortcut = self.shortcut1(X)
        out = self.relu1(out + shortcut)
        shape = out.shape
        out = out.reshape((shape[0], shape[1] * shape[2]))
    
        out2 =  self.layers2(X_immediate)
        shortcut2 = self.shortcut2(X_immediate)
        out2 = self.relu2(out2 + shortcut2)
        shape2 = out2.shape
        out2 = out2.reshape((shape2[0], shape2[1] * shape2[2]))
        
        concat = torch.cat([out, out2], 1) 
        concat = self.output(concat)   
        return concat
